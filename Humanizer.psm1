...
#>
function Add-HumanizerLibrary
{
    [CmdletBinding()]
    Param
    (
        # Path to the Humanizer.dll
    ...
    try 
    {
        ...
        else
        {
            $env:PSModulePath.split(';').foreach({ 
                $modulePath = Join-Path -Path $_ -ChildPath 'Humanizer\Humanizer.dll'
                Write-Verbose "Testing $modulePath"
                if (Test-Path -Path $modulePath)
                {
                    Write-Debug "Found $modulePath"
    ...
    {
        Write-Error "Could not load Humanizer.dll from any of the given locations."
    }
}
